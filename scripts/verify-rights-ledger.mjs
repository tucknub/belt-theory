...
for (const asset of manifest.assets) {
  ...
  if (!record) {
    failures.push(`${asset.id} is missing from IMAGE_RIGHTS_LEDGER.csv`);
    continue;
  }
  ...
  if (record.share_alike === 'Yes' && !asset.changes.toLowerCase().includes('derivatives remain')) {
    failures.push(`${asset.id} is share-alike but its transformation disclosure does not preserve the derivative license`);
  }
...
for (const row of rows) {
  if (['Candidate', 'Restricted context', 'Rejected for original ECW'].includes(row.status) && seenIds.has(row.asset_id)) {
    failures.push(`${row.asset_id} has prohibited ledger status ${row.status} but appears in the build manifest`);
  }
}

if (manifest.assets.length !== 8) failures.push(`Expected 8 homepage archival assets; found ${manifest.assets.length}`);
if (!seenIds.has('BT-ECW-003')) failures.push('The period-correct original-ECW asset BT-ECW-003 is missing');
