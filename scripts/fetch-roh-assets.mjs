...
const userAgent='BeltTheory/1.3 (+https://github.com/tucknub/belt-theory; ROH archival asset build)';
...
function dimensions(bytes,type){
  ...
  if(type.includes('jpeg')||type.includes('jpg')){
    ...
    while(o+9<bytes.length){
      ...
      const m=bytes[o+1]; const standalone=m===0xd8||m===0xd9||(m>=0xd0&&m<=0xd7);
      if(standalone){o+=2;continue;}
      const len=bytes.readUInt16BE(o+2);
      if([0xc0,0xc1,0xc2,0xc3,0xc5,0xc6,0xc7,0xc9,0xca,0xcb,0xcd,0xce,0xcf].includes(m)) return {height:bytes.readUInt16BE(o+5),width:bytes.readUInt16BE(o+7)};
      o+=2+len;
    }
  }
...
for(const asset of manifest.assets){
  ...
  for(const requestedWidth of asset.widths){
    const candidates=requestedWidth>=asset.originalWidth?[asset.originalUrl]:[thumbUrl(asset,requestedWidth),specialRedirect(asset,requestedWidth),asset.originalUrl];
    let result=null; const errors=[];
    for(const url of [...new Set(candidates)]){
      try{
        const fetched=await fetchImage(url); const size=dimensions(fetched.bytes,fetched.type);
...
await writeFile(path.join(outputDir,'roh-manifest.json'),`${JSON.stringify({version:manifest.version,builtAt:new Date().toISOString(),assets:built},null,2)}\n`);
console.log(`Archived ${built.length} rights-approved ROH photographs.`);
