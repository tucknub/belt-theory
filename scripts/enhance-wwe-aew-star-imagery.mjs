...
function credit(a, cls='ff-photo-credit') { return `<a class="${cls}" href="${esc(a.sourcePage)}" rel="noreferrer" target="_blank">${esc(a.shortCredit)}</a>`; }
...
const swerve = asset('BT-AEW-STAR-002');
...
home = setOnce(home, /(<a class="ff-story ff-story-large" href="report-wwe-vs-aew\.html"><span class="ff-story-media ff-media-pair">)[\s\S]*?(<\/span><span class="ff-story-copy">)/, `$1<img src="${cody.src}" alt="${esc(cody.alt)}" loading="lazy"><img src="${swerve.src}" alt="${esc(swerve.alt)}" loading="lazy">$2`, 'homepage WWE-AEW story imagery');
...
directory = setOnce(directory, /(<a class="ff-directory-row" href="scorecard-aew\.html"[\s\S]*?<span class="ff-directory-media"><img src=")[^"]+(" alt=")[^"]*(")/, `$1${swerve.src}$2${esc(swerve.alt)}$3`, 'Promotions AEW image');
...
const matchupMedia = `<div class="ff-matchup-media ff-matchup-media-2 ff-matchup-star-pair"><figure class="ff-matchup-cody"><img src="${cody.src}" alt="${esc(cody.alt)}" fetchpriority="high" decoding="async"><span>WWE</span>${credit(cody)}</figure><figure class="ff-matchup-swerve"><img src="${swerve.src}" alt="${esc(swerve.alt)}" fetchpriority="high" decoding="async"><span>AEW</span>${credit(swerve)}</figure></div><div class="ff-matchup-fade">`;
...
debates = setOnce(debates, /(<a class="ff-debate-row" href="report-wwe-vs-aew\.html">[\s\S]*?<span class="ff-debate-media"><img src=")[^"]+(" alt=")[^"]*(")/, `$1${swerve.src}$2${esc(swerve.alt)}$3`, 'Big Debates WWE-AEW image');
await writeFile(path.join(out, 'comparisons.html'), debates);

console.log('Enhanced WWE and AEW fan-first surfaces with six self-hosted, rights-approved champion photographs.');
